...
|-------|---------|-------------|
| **Planner** | Breaking down features, creating specs, analyzing requirements | Backend, Frontend, Data Engineer |
| **Backend** | .NET API, EF Core, authentication, caching, DTOs | Frontend, QA, Security |
| **Frontend** | React components, Redux, Tailwind, TypeScript | Backend, QA, Reviewer |
| **Data Engineer** | Python ingestion, OpenF1 sync, bulk imports | Backend, QA |
| **QA** | Unit tests, integration tests, E2E tests, coverage | Reviewer, Agent |
| **Reviewer** | Code review, pattern compliance, quality checks | Security, Agent |
| **Security** | Security audits, auth review, vulnerability assessment | Agent, Reviewer |
| **Spoiler Shield** | Implementing/reviewing the critical spoiler protection feature | Backend, Frontend, QA |

### Recommended Workflow

1. **Planning Phase**: Use `@planner` to break down complex features
2. **Implementation**: Hand off to `@backend`, `@frontend`, or `@data-engineer`
3. **Testing**: Use `@qa` to write tests for new code
4. **Review**: Use `@reviewer` for code quality, `@security` for security audit
5. **Spoiler Features**: Always involve `@spoiler-shield` for result-related features
